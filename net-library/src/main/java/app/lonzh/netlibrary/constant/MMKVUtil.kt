package app.lonzh.netlibrary.constant
...
object MMKVUtil {
    ...
    fun setToken(token: String) {
        getInstance(null)?.encode(TOKEN, token)
    }

    fun getToken(): String? {
        return getInstance(null)?.decodeString(TOKEN)
    }

    fun clearUser() {
        getInstance(null)?.remove(USER)
        getInstance(null)?.remove(TOKEN)
    }
}